...
export const LogsSearchInterface: React.FC<LogsSearchInterfaceProps> = ({ onSearch, isLoading = false }) => {
  ...
  return (
    <div className="bg-white rounded-lg border border-neutral-200 p-6">
      <h2 className="text-xl font-bold mb-4 text-neutral-900">Search Logs</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {/* Rule ID Filter */}
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-1">Rule ID</label>
          <input
            type="text"
        ...
        {/* Webhook ID Filter */}
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-1">Webhook ID</label>
          <input
            type="text"
        ...
        {/* Search Term */}
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-1">Search (error message, etc.)</label>
          <input
            type="text"
      ...
      <div className="flex gap-2">
        <button
          ...
          disabled={isLoading}
          className="px-4 py-2 bg-primary-600 text-white rounded-md text-sm font-medium hover:bg-primary-700 disabled:bg-neutral-400"
        >
          {isLoading ? 'Searching...' : 'Search'}
        </button>
        <button
          onClick={handleReset}
          className="px-4 py-2 bg-neutral-200 text-neutral-800 rounded-md text-sm font-medium hover:bg-neutral-300"
        >
          Reset
        </button>
      </div>
